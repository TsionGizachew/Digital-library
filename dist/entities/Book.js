...
const bookSchema = new mongoose_1.Schema({
    ...
    status: {
        type: String,
        enum: Object.values(enums_1.BookStatus),
        default: enums_1.BookStatus.AVAILABLE,
    },
    coverImage: {
        type: String,
        default: null,
...
bookSchema.pre('save', function (next) {
    ...
    if (this.availability.availableCopies === 0) {
        this.status = enums_1.BookStatus.BOOKED;
    }
    else if (this.status === enums_1.BookStatus.BOOKED && this.availability.availableCopies > 0) {
        this.status = enums_1.BookStatus.AVAILABLE;
    }
    this.metadata.popularityScore = this.calculatePopularityScore();
    next();
});
bookSchema.methods.isAvailable = function () {
    return this.status === enums_1.BookStatus.AVAILABLE && this.availability.availableCopies > 0;
};
