...
const AnnouncementsPage: React.FC = () => {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'urgent': return 'bg-error-500 text-white shadow-md';
      case 'high':   return 'bg-accent-500 text-white shadow-md';
      case 'medium': return 'bg-primary-500 text-white shadow-md';
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8" role="main" aria-label="Library announcements">
            ...
            {announcements.map((announcement) => (
              ...
              >
                  ...
                  <div className="relative h-40 sm:h-48 bg-gradient-to-br from-primary-50 to-brand-50 dark:from-primary-950 dark:to-brand-950 rounded-lg mb-3 sm:mb-4 overflow-hidden shadow-md group-hover:shadow-xl transition-shadow duration-300" aria-hidden="true">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-xs sm:text-sm font-medium text-neutral-500 dark:text-neutral-400">Announcement</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-start justify-between mb-2 sm:mb-3">
                  ...
                  <div className="bg-gradient-to-br from-primary-500 to-brand-500 rounded-full p-1.5 sm:p-2 shadow-md" aria-hidden="true">
                    <MegaphoneIcon className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
                  </div>
                </div>
                
                {/* Enhanced Title */}
                <h3 className="text-base sm:text-lg font-bold text-neutral-900 dark:text-neutral-100 mb-2 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors duration-200 line-clamp-2 break-words">
                  {announcement.title}
                </h3>
                ...
                <div className="flex items-center justify-between gap-2 text-xs text-neutral-500 dark:text-neutral-400 mt-auto pt-3 border-t border-neutral-200 dark:border-neutral-700">
                  <div className="flex items-center gap-1.5 bg-neutral-50 dark:bg-neutral-800 rounded-lg px-2 py-1">
                    <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <span className="font-medium">{new Date(announcement.publishDate).toLocaleDateString()}</span>
                  </div>
                  <div className="flex items-center gap-1.5 bg-primary-50 dark:bg-primary-950 rounded-lg px-2 py-1">
                    <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-brand-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {!loading && announcements.length === 0 && (
          <div className="text-center py-12 sm:py-16">
            <div className="bg-gradient-to-br from-primary-500 to-brand-500 rounded-full p-4 sm:p-5 mx-auto mb-3 sm:mb-4 w-fit shadow-xl">
